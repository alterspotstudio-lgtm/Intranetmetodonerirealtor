...
// Valida usuario por Slug en Airtable y entrega token firmado.
...
// Campo de contraseña opcional. El reporte real de Asesor Captación NO trae contraseña,
// por eso el login también acepta una contraseña temporal automática derivada del Slug.
...
export default async function handler(req, res){
  ...
  try{
    ...
    if(!rec) return res.status(401).json({ error:'No se encontró asesor con ese slug.' });

    const f = rec.fields || {};
    const activo = truthy(f['Activo']);
    if(!activo) return res.status(403).json({ error:'El asesor no está activo. Revisa activación en Control de Mando.' });

    const fieldPass = PASS_FIELDS.map(k=>f[k]).find(Boolean);
    const tempPass = tempPassword(field(f,'Slug') || user);
    const globalCode = process.env.NERI_ACCESS_CODE;
    const passOk = fieldPass ? String(fieldPass) === pass : (pass === tempPass || (globalCode && String(globalCode) === pass));
    if(!passOk){
      return res.status(401).json({ error: fieldPass ? 'Contraseña incorrecta.' : 'Contraseña temporal incorrecta.' });
    }

    const independiente = truthy(f['Independiente']);
    const puesto = field(f,'Puesto') || field(f,'Puesto (texto viejo)');
    const tipo = inferTipo(independiente, puesto);
    const rol = normalizeRole(rolSolicitado, tipo, puesto);
    const payload = basePayload({
      user,
      slug: field(f,'Slug') || user,
      rol,
      nombre: field(f,'Nombre') || user,
      whatsapp: onlyDigits(field(f,'Teléfono WhatsApp') || field(f,'Telefono WhatsApp')),
      email: field(f,'Email'),
      ciudad: field(f,'Ciudad') || field(f,'Ciudad (texto viejo)'),
      empresa: independiente ? 'N/A' : 'Inmobiliaria',
      cuenta: independiente ? 'N/A' : 'Inmobiliaria',
      tipo,
      estado: 'Activo',
      puesto,
      pixel: field(f,'Pixel ID Meta'),
      linkCaptacion: field(f,'Link Captación') || `https://captacion.vercel.app/?asesor=${encodeURIComponent(field(f,'Slug') || user)}`,
      recId: rec.id,
      mustChangePassword: !fieldPass,
      authMode: fieldPass ? 'airtable-field' : 'temporary-derived'
    });
    return res.status(200).json({ token: sign(payload), user: payload });
  }catch(err){
    return res.status(500).json({ error:'Error al iniciar sesión: '+(err?.message || err) });
  }
}
...
function inferTipo(independiente, puesto){
  const p=String(puesto||'').toLowerCase();
  if(independiente && (p.includes('gerente') || p.includes('director'))) return 'Grupo de asesores sin inmobiliaria';
