...
function applyReadScope(parsed,tableCfg,session){ if(isAdmin(session)) return; if(tableCfg.slugField){ mergeFormula(parsed.params,`{${tableCfg.slugField}}='${escFormula(session.slug)}'`); return; } if(tableCfg.ownerField){ const owner=session.nombre||session.slug; mergeFormula(parsed.params,`{${tableCfg.ownerField}}='${escFormula(owner)}'`); } }
...
function applyWriteScope(req,tableCfg,session){ const body=normalizeBody(req); if(isAdmin(session)||isManager(session)) return; if(tableCfg.slugField){ body.fields[tableCfg.slugField]=session.slug; stripLockedProfileFields(body.fields); return; } if(tableCfg.ownerField){ const attempted=body.fields[tableCfg.ownerField]; const own=session.nombre||session.slug; if(attempted&&fieldValue(attempted)!==own) throw new Error('No puedes crear registros para otro asesor.'); body.fields[tableCfg.ownerField]=own; } }
async function applyPatchScope(req,parsed,tableCfg,session){ const body=normalizeBody(req); if(isAdmin(session)||isManager(session)) return; if(!parsed.recordId) throw new Error('PATCH requiere recordId.'); const rec=await readRecord(parsed.table,parsed.recordId); if(tableCfg.slugField){ if(fieldValue(rec.fields?.[tableCfg.slugField]).toLowerCase()!==String(session.slug||'').toLowerCase()) throw new Error('No puedes editar el perfil de otro asesor.'); stripLockedProfileFields(body.fields); return; } if(tableCfg.ownerField){ const owner=fieldValue(rec.fields?.[tableCfg.ownerField]); const own=session.nombre||session.slug; if(owner&&owner!==own) throw new Error('No puedes editar registros de otro asesor.'); if(body.fields[tableCfg.ownerField]&&fieldValue(body.fields[tableCfg.ownerField])!==own) throw new Error('No puedes reasignar registros a otro asesor.'); } }
