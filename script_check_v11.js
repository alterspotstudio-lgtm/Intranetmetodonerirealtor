...
/* ═══════════════════════════════════════════════════
   ADMINISTRACIÓN BASE V11 — Fichas cerradas por defecto + soporte técnico
   Regla de oro: al entrar a Inmobiliarias solo se ven inmobiliarias.
   La creación solo vive en + Crear nueva intranet.
═══════════════════════════════════════════════════ */
const ADMIN_V9_STORAGE_KEY = 'neri_admin_base_v11_fichas_cerradas';
...
function adminRenderCuentaDetalle(){
  ...
  const users = adminCuentaUsers(c.id);
  const selectedUser = users.some(u=>u.id===adminState.selectedUsuarioId) ? adminUsuario(adminState.selectedUsuarioId) : null;
  ...
  return `<div class="admin-v9-ficha">
    ...
    <div class="admin-v9-ficha-body">
      ...
      <div class="admin-v9-panel"><div class="admin-v9-panel-head"><div><div class="admin-v9-panel-title">USUARIOS <em>DEL PAQUETE</em></div><div class="admin-card-sub">Lista real de usuarios: director, gerente y asesores</div></div>${c.tipo==='Asesor independiente'?'':`<button class="btn btn-primary" onclick="adminV9AddUser('${c.id}')" ${!canAdd?'disabled':''}>+ Simular registro</button>`}</div><div class="admin-v9-panel-body">
        ...
        ${selectedUser ? adminUsuarioSoporteDetalle(selectedUser) : adminUsuarioClosedNoticeV11(c)}
        <div style="height:14px"></div>
...
function adminUsuarioSoporteDetalle(u){
  ...
  return `<div class="admin-v9-user-detail"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><h4>FICHA EDITABLE DE USUARIO · <em>${adminEsc(u.nombre)}</em></h4><button class="admin-v9-smallbtn" onclick="adminCloseUsuarioV11()">Cerrar ficha</button></div>
    <div class="admin-v9-form-note">Para administración, director, gerente y asesor cuentan igual como usuario del paquete. El puesto solo cambia sus privilegios dentro de la intranet.</div>
    <div class="admin-form-grid">
      ...
      <div class="form-group"><label>Slug automático oficial</label><input class="form-inp" value="${adminEsc(u.slugOficial||'Pendiente')}" disabled></div>
      <div class="form-group"><label>Link Captación generado</label><input class="form-inp" value="${adminEsc(adminLink(u))}" disabled></div>
    </div>
    <div style="height:12px"></div>
    <div class="admin-card-sub" style="margin-bottom:8px">Servicios / permisos de este usuario</div>
    <div class="admin-v9-checks">${checks}</div>
...
function adminOpenUsuarioV9(id){ adminState.selectedUsuarioId = id; adminRender(); }
function adminCloseUsuarioV11(){ adminState.selectedUsuarioId = null; adminRender(); }
...
function adminV9ToggleUser(id){ const u = adminUsuario(id); if(!u) return; u.estado = u.estado === 'Inactivo' ? 'Activo' : 'Inactivo'; adminSave(); adminRender(); }
function adminV9DeleteUser(id){ const u = adminUsuario(id); if(!u) return; const c = adminCuenta(u.cuentaId); if(c?.tipo === 'Asesor independiente'){ showToast('La cuenta independiente requiere su perfil único'); return; } adminState.data.usuarios = adminState.data.usuarios.filter(x=>x.id!==id); adminState.selectedUsuarioId = null; adminSave(); adminRender(); }
